...
const ArtistInfoStep: React.FC<ArtistInfoStepProps> = () => {
  const { state, updateField } = useSmartLinkForm();const handleChange = (field: keyof typeof state, value: string) => {
  ...
  // Cleanup blob URLs quando o componente desmonta
  useEffect(() => {
    return () => {
        ...
        <div className="space-y-4 order-1 md:order-1">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            ...
            <div>
              ...
              >
                <option value="noite-carioca">🌃 Noite Carioca - Boate e Luxo</option>
              </select>
            </div>
          </div>
        </div>
